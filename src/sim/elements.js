// Every element, reaction and recipe in the game is defined in this file.
// The simulation, the palette and the recipe book are all generated from
// these tables, so a recipe that exists here is guaranteed to be something
// the simulation can actually do (see test/recipes.test.js).
...
export const ELEMENT_LIST = [
  ...
  },
  {
    ...
    density: 1.0, conduct: 0.3, spread: 8,
  ...
  // ---- first discoveries -------------------------------------------------
  {
    ...
    colors: ['#c9d6e3', '#b8c7d6', '#d4dfea'], density: 0.05, temp: 110, conduct: 0.1,
  ...
  {
    key: 'MOLTEN_GLASS', name: 'Molten Glass', sym: 'Mg', cat: 'liquid', state: LIQUID,
  ...
  },
  {
    ...
    colors: ['#9fd3e6', '#b0dcec', '#94cbe0'], density: 2.5, conduct: 0.1, acidProof: true,
  ...
  {
    key: 'PLANT', name: 'Plant', sym: 'Pl', cat: 'solid', state: SOLID,
  ...
  },
  {
    ...
    colors: ['#6b6b70', '#5f5f64', '#77777c'], density: 0.08, temp: 120, conduct: 0.05,
  ...
  },
  {
    ...
    colors: ['#2f94b8', '#3399bd', '#2a8bad', '#3aa2c4'], density: 1.1, conduct: 0.3, spread: 8,
  ...
  {
    key: 'DIAMOND', name: 'Diamond', sym: 'Dm', cat: 'solid', state: SOLID,
    colors: ['#bff4ff', '#d8fbff', '#a6ecfa', '#e8feff'], density: 3.5, conduct: 1.0,
    acidProof: true, sparkle: true,
  ...
  {
    key: 'METAL', name: 'Metal', sym: 'Fe', cat: 'solid', state: SOLID,
    colors: ['#8e9aa8', '#86929f', '#98a4b1', '#7f8b98'], density: 7.8, conduct: 0.9,
    conductor: true,
    high: { temp: 1538, to: 'MOLTEN_METAL', chance: 0.05 },
  ...
  },
  {
    ...
    low: { temp: 1450, to: 'METAL', chance: 0.05 },
    desc: 'Liquid iron. Cast it into any shape.',
  ...
  },
  {
    ...
    high: { temp: 1538, to: 'MOLTEN_METAL', chance: 0.02 },
  ...
  },
  {
    ...
    flammable: 0.08, ignite: 250, burn: { smoke: 0.6, fireLife: [60, 120] },
    desc: 'Floats on water and burns readily.',
    hint: 'Compress Plants.',
  },
  {
    key: 'METHANE', name: 'Methane', sym: 'Me', cat: 'gas', state: GAS,
    colors: ['#9fc2a0', '#93b694'], density: 0.04, conduct: 0.05, rise: 0.55, sink: 0.05,
  ...
  {
    key: 'GUNPOWDER', name: 'Gunpowder', sym: 'Gp', cat: 'powder', state: POWDER,
  ...
  },
  {
    ...
    colors: ['#d6e9ff', '#c9e1ff'], density: 0.01, conduct: 0.2, rise: 0.8, sink: 0.03,
  ...
  },
  {
    ...
    colors: ['#dfe4ea', '#d2d8df', '#e9edf1'], density: 0.06, temp: 10, conduct: 0.1,
  ...
  {
    key: 'THERMITE', name: 'Thermite', sym: 'Th', cat: 'powder', state: POWDER,
  ...
  {
    key: 'NITRO', name: 'Nitro', sym: 'Ni', cat: 'liquid', state: LIQUID,
  ...
  },
  {
    ...
    conduct: 0.5, spread: 5, life: [400, 700], behavior: 'decay',
    desc: 'Liquid at −196 °C. Freezes what it touches, then boils away.',
  ...
  },
  {
    ...
    colors: ['#120a1c', '#1a0f28', '#0d0715'], conduct: 0, acidProof: true, behavior: 'void',
...
export const REACTIONS = [
  ...
  { a: 'WATER', b: 'SPARK', chance: 0.15, aTo: 'HYDROGEN', bTo: null },
...
export const ID = {};
ELEMENT_LIST.forEach((e, i) => { ID[e.key] = i; });
...
function normalize(e, i) {
  ...
  d.displaceable = e.state === LIQUID || e.state === GAS || (e.state === ENERGY && !d.fixed);
...
for (const r of REACTIONS) {
  ...
  const aTo = id(r.aTo), bTo = id(r.bTo);
  const aRule = aTo >= 0 ? addRule('contact', [a, b], aTo) : -1;
  const bRule = bTo >= 0 ? addRule('contact', [a, b], bTo) : -1;
  REACT[a * NUM + b] = {
    chance: r.chance, selfTo: aTo, otherTo: bTo, keepSelf: !!r.keepA, keepOther: !!r.keepB,
    selfRule: aRule, otherRule: bRule,
  };
  REACT[b * NUM + a] = {
    chance: r.chance, selfTo: bTo, otherTo: aTo, keepSelf: !!r.keepB, keepOther: !!r.keepA,
    selfRule: bRule, otherRule: aRule,
  };
